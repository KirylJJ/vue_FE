<template>
    <v-data-table v-model="selected" :headers="headers" :items="allTodos" class="elevation-1">
        <template v-slot:item.status="{ item }">
            <v-checkbox v-model="item.status" color="success"></v-checkbox>
        </template>

        <template v-slot:item.status_desc="{ item }">{{item.status ? 'Done' : 'Undone'}}</template>
        ...
        <template v-slot:item.title="{ item }">
            <span @click="tableRowTitleClick(item)" class="title-btn">{{ item.title }}</span>
        </template>
...
export default {
    ...
    methods: {
        ...
        async deleteItem(item) {
            await this.deleteTodo(item.todo_id);
